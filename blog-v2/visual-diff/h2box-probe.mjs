...
for (const [i, origin] of ['http://127.0.0.1:8801', 'http://127.0.0.1:8802'].entries()) {
  ...
  const r = await page.evaluate(() => {
    ...
    const kids = [...h.childNodes].map(n => {
      const range = document.createRange()
      range.selectNodeContents(n)
      return `${n.nodeType === 3 ? 'TEXT' : n.nodeName} ${JSON.stringify(n.textContent.slice(0, 25))} rects=${[...range.getClientRects()].map(r => `y${r.top.toFixed(1)}h${r.height.toFixed(1)}`).join(',')}`
    })
    return { pt: cs.paddingTop, pb: cs.paddingBottom, bb: cs.borderBottomWidth, mt: cs.marginTop, mb: cs.marginBottom, fs: cs.fontSize, lh: cs.lineHeight, h: h.getBoundingClientRect().height, kids }
  })
