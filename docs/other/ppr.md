...
比如说鹅厂的王牌游戏-王者荣耀的好友推荐就是基于PPR的 ~~(一般人我不跟他说)~~
...
* `蒙特卡洛`就是: 从100个🍎s中挑最大的，拿一个在手上，再随机挑一个，选二者最大的，除非遍历完毕，否则只能给出一个**近似**最优解
* `拉斯维加斯`就是: 从100把🔑中找到能开门的钥匙，不能保证一定找得到最优解，但找到了肯定是最优解

那么这里的`MC算法`就是以随机游走的概率估计PPR值 ~~(其实相同的方法我们在PageRank的计算中也提到过)~~

那么估计都是一个无偏估计 每次Random walk都是对所有点的无偏估计！

直观上感觉Random walk越多估计的越准

对固定一个点 每次Random Walk的结果之间是独立的
...
达到0误差是非常困难 而且没有必要的 所以需要追求误差比较小$\le \epsilon$ 得到近似正确的概率比较大$\ge 1-\delta$
...
有Forward 很容易想到是不是有`Backward`

此时的$\pi_b(v,t), r_b(v,t)$就和Forward的基本一致
...
回顾刚才的过程 可以发现 我们在重试的过程中消耗太多 有没有什么办法能减少重试次数呢

刚才我们把所有情况都列成一列 那么是不是有其和=1

如果我们按均值进行归一化 是不是所有方块的均值应该为1
...
且随着$\epsilon$值的增大，收敛速度越快 (越激进越有可能发现真理 所以同学们 要保持对世界的怀疑)

但$\epsilon$-Greedy 忽略了可能已经出现的特征 从始至终的进行随机筛选

当然也有一些$\epsilon$-Greedy 的变种 比如说一开始尝试概率高 之后概率慢慢减小; 通过预筛选 先框定小范围 再进行$\epsilon$-Greedy
...
实际上 SoftMax的最大特点就是通过一个变量T，Temperature来控制估计的范围力度

T-温度，直观的感受，随着时间的增大，T随着减小 那么在分母的T导致现有的样本越重要

另外SoftMax也有一些变体比如说$p_k=(1-\gamma)\dfrac{w_k(t)}{\sum \limits_{j=1}^K w_j(t)}+\dfrac{\gamma}{K}$, 其中$w_j(t+1)=w_j(t)exp(\gamma \dfrac{r_j(t)}{p_j(t)K})$

#### UCB

虽然`SoftMax`已经有一种尝试越多 估计越可用的感觉 但他没有考虑到置信区间的问题 UCB则从置信区间出发
...
和SoftMax相比 只是$p_k$计算方法略有区别 然后还必须进行一次预操作处理

仔细观察$p_k$式子，其中包含了试验次数

随着试验次数的增大后面那项值越小，代表越接近真实，而试验次数较小的时候，其值较大，从而中和不太准的第一项
...
而`Thompson sampling`则是站在贝叶斯的角度 通过维护一个概率模型用先验估计后验
...
利用Chernoff bound估计所有点的UB LB且所有点的$UB-LB$均相等
