...
1. 利用`wordHash`代替传统词袋模型 从而达到降维效果
    ...
    + 每个切割分量作为一维向量
    + 因为英文中单词数量级相对于n个字母组成单词的数量级较大
    + 且这种方案的Hash碰撞率较小 3字母表示仅为`0.0044％`
    + 这种思想可以看做是`Word2Vector`早期的思想
    + 每个词之间并非完全正交
...
随着word2Vec的提出 再加上NN方法在NLP中进一步运用 检索式QA有了进一步的发展

但回顾之前的DSSM模型 在计算出句向量之后就直接使用cosine distance 进行计算Similarity
...
文章给出了三种对齐函数的计算方式 1. 该单词是否存在于另一个句子中 2. 词向量直接点积 3.词向量计算余弦相似度
...
然后这种多粒度思考词、句之间的做法 之后发展成交互的思想 广泛应用于目前前沿检索式QA模型中
...
慢慢的大家发现 仅仅从词的角度 去进行检索式QA不能达到很好的效果 尤其是在多轮对话中效果并不好 于是效果不错的交互式就越来越流行
...
> 这种多粒度的做法 保证了即使CNN很浅，也能抽取出比较high-level的特征，得到高质量的utterance embedding
...
* 另外一个矩阵M2 是先`self-attentation` 然后和embedding之前拼接起来 再过一次`GRU`
